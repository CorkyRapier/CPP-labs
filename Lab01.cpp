...
public:
	...
	Stock(int valuekol_ak, string valueName) {
		...
		S = 0.0;
		G = 0.0;
		cout << "Вызываем конструктор с параметрами" << endl;
	}
	...
	void acquire(string cname) {
		cout << "Введите изначальное кол-во акций: ";
		cin >> kol_ak;
		Name = cname;
		for (int i = 0; i < kol_ak; i++) {
			double p;
	...
	void show() {
		cout <<"Компания "<< Name << " имеет в пакете акций: " << kol_ak << endl << "Цена каждой акции: ";
		for (int i = 0; i != kol_ak; i++) {
			if (i == kol_ak -1) {
				cout << price[i] << ". ";
...
int main()
{
	...
	string faqq;
	Student Ivanov("Иван", "Иванов", "Иванович", "adress", "ИиВТ", 2001, 89546721891, 3);
	Smass.push_back(Vetrov);
	kol_vo++;
	Student Petrov("Петр", "Петров", "Петрович", "adress", "ИиВТ", 2001, 89546721891, 3);
	Smass.push_back(Chichkanov);
	kol_vo++;
	Student Vasilev("Висилий", "Васильев", "Васильевич", "adress", "ИСИТ", 2000, 89546721891, 4);
